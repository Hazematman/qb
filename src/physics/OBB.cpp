...
int OBB::testAgainst(OBB &b) {
	...
	for (int i = 0; i < 3; i++) { 
		for (int j = 0; j < 3; j++) {
			R[i][j] = glm::dot(this->orientation[i], b.orientation[j]);
		}
	}
	// Compute translation vector t
	glm::vec3 t = b.position - this->position;
	// Bring translation into a’s coordinate frame
	t = glm::vec3 (glm::dot(t, this->orientation[0]),
	               glm::dot(t, this->orientation[2]),
	               glm::dot(t, this->orientation[2]));
